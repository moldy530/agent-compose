...
| `outcome` | `"completed"` \| `"refused"` \| `"failed"` | always | What the harness's loop did with the call. `"completed"` handed its model the tool's result; `"refused"` is the harness's own permission surface declining the call, which is what a node's `allow_tools:` produces on the harness that enforces it in-loop (grammar §8.9); `"failed"` is the call's execution failing. On `cc` a refusal is taped `"refused"` only where this runtime's permission callback answered it, which the node's permission mode decides (grammar §8.9): under `default`, `acceptEdits` and `plan` it does, and under `auto` where the mode's classifier hands the question back. A call the SDK denies without consulting the callback — any denial under `dontAsk`, which consults none, and one `auto`'s classifier makes itself — comes back to the loop as an error result and is recorded `"failed"`, though it never executed. A refusal did **not** end the run — the harness's loop went on, exactly as an agent's does after a refusal (§7.3). |
