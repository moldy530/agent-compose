...
- **Built-in tools** (owner call, 2026-08-22: both wanted, both opt-in):
  ...
  bash/file tools (gated on §10 q31).
...
31. **What runtime built-in tools (bash, file I/O) are, and what bounds them.** Owner scope call (2026-08-22): wanted, and opt-in. An author can already hand-roll a shell through a `tool.*`'s `exec:`; a curated set removes the boilerplate, but unlike resolved q30 this is a security decision — a model holding bash is arbitrary code execution on the host running the graph. Lead recommendation: **explicit opt-in per agent node** by listing each tool (`builtin.bash`, `builtin.read_file`, `builtin.write_file`, `builtin.list`), never ambient and never a single switch that grants the set; **file tools take a mandatory `root:`** and refuse any path that resolves outside it; **bash runs with that root as its working directory and a timeout**, its schema refusals bouncing per resolved q22 and its execution failures failing the node like every `exec:` tool; and **v1 containment is root + timeout only** — the tool runs with the runtime's own privileges, as any hand-rolled `exec:` tool does today, with containers/seccomp deferred until a deploy story needs them (awaiting owner's call on whether that is hard enough). Sub-questions: the exact tool set and each tool's schema; whether a deploy target may *restrict further* (refusing bash on a distributed placement reads like a placement fact); trace treatment of outputs — recorded like any tool result so replay (resolved q29) consumes them, but a bash tool's output can be large and can carry anything on the host, so the excerpt/redaction policy needs stating against `docs/trace.md` §11; and the shell posture on non-POSIX hosts.
