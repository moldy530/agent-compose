...
- **Server tools** (resolved q30): a provider MAY declare `server_tools:`, an array of config objects in that provider's *own wire vocabulary*, appended verbatim to the `tools` of every request that connection serves. The runtime dispatches nothing for them — they execute on the provider's side, inside the model call — so they are provider config rather than a construct of this grammar, and replay is untouched. Checking is **two-tier**: a curated per-`kind` table is enforced strictly, and anything outside it is a warning naming what could not be verified and is carried to the wire unchanged, so a tool the vendor ships after this compiler release is usable the day it ships. A suite belongs to a **connection**, which is why differing suites across a `route:`'s members are a warning rather than a refusal, and why declaring one can move the connection's wire (an `openai` provider with a suite speaks the Responses API for all of its calls).
...
- **Server tools**: a provider's `server_tools:` pass through to every request
  that connection serves, checked in two tiers, with the Responses wire an
  `openai` provider moves to when it declares one (resolved q30).
