...
30. **What a provider-executed ("server") tool is in the grammar** → **an array of wire-shaped tool configs, declared on the provider and passed through, under two-tier static checking.** Owner ratification (2026-08-22), and the owner set the shape — the governing constraint is *no manual support treadmill*: a server tool the vendor ships tomorrow must be usable the day it ships, without waiting for a compiler release. So a provider definition carries an array of tool config objects written in that provider's own wire vocabulary, and the runtime appends them to the `tools` array of every request that provider serves — the runtime dispatches nothing, since a server tool executes on the provider's side, inside the model call, and its results arrive woven into the assistant's turn. **The checking is two-tier.** The compiler keeps a curated table of *known* server tools per provider kind, and a config naming one is validated strictly — a tool the kind does not have, a misspelled or mistyped field, a constraint violation are all errors with G3 diagnostics naming the repair. A config naming a tool *outside* the table is a **warning, not an error** — it names exactly what could not be verified, then travels to the wire verbatim. Refusing unknowns instead would recreate the treadmill this resolution exists to refuse; erroring on knowns keeps a typo in today's tools failing at `validate` rather than at runtime. **Launch scope is the full server-tool suite for the `anthropic` and `openai` kinds** in the known table — for Anthropic's Messages wire that is web search, web fetch, and code execution; for OpenAI it is the built-in tool suite of the **Responses API** (web search, file search, code interpreter, image generation), which Chat Completions does not carry — so this resolution knowingly brings the Responses wire into the generated runtime and the mock provider as part of the work. **Declaring on the provider** (rather than the lead's earlier per-agent-node recommendation — superseded, owner call) has two stated consequences: every agent whose model resolves to that provider holds its tools, and scoping a tool to one agent is done by defining a second provider, which is cheap config; and failover capability is per-chain-member *by construction* — each provider in a chain declares its own array, so `validate` **warns** when chain members declare differing suites (the capability change on failover is made visible rather than refused, since a failover target without web search may be exactly what a deployment intends). Replay is untouched: the use happens inside the recorded model call, so durability (resolved q29) consumes it with the recorded answer. The mock provider must be able to script server-tool-bearing responses on both wires so the acceptance harness covers this without keys (5.9, 5.12, G3).
