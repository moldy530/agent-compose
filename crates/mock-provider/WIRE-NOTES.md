...
*If wrong*: a structured reply is refused as a `script-mismatch` (409, header
`x-mock-provider-error`), which is loud and points here.
...
### 11. The harness's own refusals use a status no provider sends

An unscripted call, a script that cannot be rendered, and a control-plane
document that will not parse all answer **409** with an
`x-mock-provider-error` header, wrapped in the surface's own error envelope so a
client library can still parse them. 409 is deliberate: PRD 5.9 fails over on
429/5xx/no-answer, so a harness refusal wearing one of those statuses would be
silently retried and the run would report a different failure than the one it
had. The envelope's `type` is the surface's `invalid_request_error` rather than
an invented one, because an unknown error type is what a client library least
reliably parses.
*If wrong* (a client retries 409s): the transcript still shows the refusal, and
the run fails with the queue empty rather than passing.
