...
//! | `a-cross-quote-escape-keeps-its-backslash` | `'…\"…'` and `"…\'…"` — a quote escaped inside the *other* quote's literal | the crate keeps the backslash (`'\"'` is **two** characters there), this reads the escape (`'\"'` is `"`) | the specification's escape table admits `\"` and `\'` in either literal and gives both one meaning, the quote; the crate's `parse_quoted_string` pushes the backslash back for the redundant spelling, which is a defect inherited from the `snailquote` code its own comment credits. The same choice as `size-of-a-string-counts-code-points` and for the same reason: the compiler never evaluates, so the crate is the corpus's reference column rather than a runtime, and the evaluator that actually runs follows the specification. `escapes.json` states the two **non**-redundant spellings, where the columns agree, and `a-quote-escaped-inside-the-other-quotes-literal-still-keeps-its-backslash` pins the crate's answers so an upstream fix or a pin bump goes red |
//!
//! Every other escape agrees, and `escapes.json` is what says so: the octal
//! `\OOO`, `\a`, `\?`, the backtick, `\xHH`, `\uHHHH`, `\UHHHHHHHH`, and the
//! rejections — `'\0'`, `'\400'`, `'\z'`, a lone surrogate — stated as cases in
//! both directions, because a lexer is exactly the place where two readings
//! diverge silently over an ordinary literal. Where the crate refuses what the
//! specification admits (`b'\n'`; `\a` in a bytes literal), nothing reaches a
//! router: `crate::cel` compiles every expression with the crate before a
//! project is emitted, so its refusal is the compiler's.
//!
