...
pub fn scaffolds(ir: &Ir) -> Vec<Scaffold> {
    ...
    crate::check::modules::bindings(ir)
        ...
        .map(|(address, module)| Scaffold {
            ...
            contents: stub(ir, &names, address, module),
...
/// One stub's bytes.
fn stub(ir: &Ir, names: &Names, address: &str, binding: &crate::ir::binding::Module) -> String {
    ...
    // Named only where there is one to name. A binding with no `env:` has a
    // type that admits no property, and pointing an author at it would be
    // advice about a surface this tool does not have.
    if !binding.env.is_empty() {
        doc.push(String::new());
        doc.push(format!(
            "The environment this binding declared arrives as the **third argument**, typed \
             `{environment}` — not through `process.env`, which holds whatever the process was \
             started with rather than what this tool declared."
        ));
    }
...
"#;
    ...
    /// A binding that declares `env:` gets one line the others do not: where the
    /// environment arrives.
    ///
    /// The stub's signature takes `input` alone — a function of fewer arguments
    /// satisfies the contract — so an author filling one in has no way to
    /// discover the third parameter from the code in front of them, and
    /// `process.env` is exactly what they would reach for instead. A binding
    /// with no `env:` is told nothing, because its environment type admits
    /// nothing.
    #[test]
    fn a_stub_says_where_a_declared_environment_arrives() {
        let with_environment = ir_of(
            r#"version: "0.1"
...
"#,
        ...
        let stub = &scaffolds(&with_environment)[0].contents;
        assert!(
            stub.contains("typed `ToolSignModuleEnv`"),
            "the stub names the environment type: {stub}"
        );
        assert!(
            stub.contains("third argument"),
            "…and where it arrives: {stub}"
        );

        let plain = &scaffolds(&ir_of(ONE_MODULE_TOOL))[0].contents;
        assert!(
            !plain.contains("third argument"),
            "a binding with no `env:` is told nothing about one: {plain}"
        );
    }
