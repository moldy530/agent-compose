...
pub fn module(ir: &Ir, partition: &Partition) -> super::GeneratedFile {
    ...
    contents.push_str(&format!(
        "\nexport const deployTarget: string = {};\n",
        names::string(&ir.deploy.target)
    ));

    // Which backend this target's execution journal binds (grammar §14.7, PRD
    // resolved q62). Here rather than in `src/journal.ts` for the reason every
    // other deploy fact is: that module is a compiler constant in its invariant
    // half, and what the *target* said belongs in the one module that carries
    // what the target said. The variable is a **name**, like every credential
    // this artifact holds (grammar §4.3).
    let provider = crate::ir::deploy::journal_of(ir);
    contents.push_str("\nexport const journal: JournalBinding = { provider: ");
    contents.push_str(&names::string(provider.as_str()));
    if let Some(url) = ir
        .deploy
        .journal
        .as_ref()
    ...
    {
        contents.push_str(", urlEnv: ");
        contents.push_str(&names::string(&url.value.name));
    }
    contents.push_str(" };\n");

    contents.push_str("\nexport const traceSink: TraceSink | undefined = ");
    match ir.deploy.trace_sink.as_ref() {
        None => contents.push_str("undefined;\n"),
        Some(sink) => {
            contents.push_str("{\n");
            contents.push_str(&format!("  url: {},\n", names::string(&sink.url.value)));
            contents.push_str(&format!(
                "  format: {},\n",
                names::string(sink.format.as_str())
            ));
            if let Some(auth) = &sink.auth {
                contents.push_str("  auth: {\n");
