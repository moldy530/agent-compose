...
const GRAMMAR: &[Check] = &[
    ...
    Check {
        ...
        rule: "a `coder:` node binds a harness this release lowers, its `model:` is a direct binding whose provider speaks that harness's wire and every fact of whose connection has a slot on it and is spelled once, its `permission_mode:` is one the bound harness has an axis for and its `access:` level admits, and its `settings:` are checked in two tiers with the reserved keys refused (8.9, D136, D140, D141, D143, D146)",
        pass: "check/coder.rs",
        codes: &[
            "unsupported-harness",
    ...
    Check {
        ...
        rule: "two harness runs that can be in flight at once are not contained by one directory: a `map`-dispatched `coder:` node's `workspace:` reads the per-dispatch scope — the item's own fields, or a state channel a node that dominates it writes (10.1, 7.6.4) — or the map is serial, and two runs concurrent branches contain — a `coder:` node, or one inside a flow a `flow:` node instantiates or a `map` node dispatches — do not write one workspace (8.9, 8.6, 7.6.1, D147)",
