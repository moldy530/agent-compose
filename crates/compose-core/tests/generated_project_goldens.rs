...
/// The authored half of a golden is the **source project's own bytes**, carried
/// (PRD resolved q49).
///
/// The test above compares a golden directory against what the emitter answers,
/// and would be satisfied by an emitter that invented a plausible file. This is
/// the other end of the same claim: what the golden holds at a `module:`
/// binding's path is byte-for-byte what is committed beside that composition's
/// `main.yml`, which is where the author edits it and where a scaffold lands.
///
/// It also says the corpus **reaches** a module binding at all — a golden set
/// with none would pass every test in this file while proving nothing about the
/// contract `tsc` is supposed to hold.
#[test]
fn a_carried_file_is_the_source_projects_own_bytes() {
    if regenerating() {
        return;
    }
    ...
    for golden in GOLDENS {
        let project = emitted(golden);
        let root = goldens::goldens_root().join(golden.directory);
        let source = goldens::repository().join(golden.project);
        for file in project.carried() {
            carried += 1;
            let authored = source.join(file.path.replace('/', std::path::MAIN_SEPARATOR_STR));
            assert_eq!(
                fs::read_to_string(&authored)
                    .unwrap_or_else(|error| panic!("cannot read {}: {error}", authored.display())),
                file.contents,
                "`{}` carries something other than what `{}` holds",
                golden.directory,
                authored.display()
            );
            assert!(
                root.join(file.path.replace('/', std::path::MAIN_SEPARATOR_STR))
                    .is_file(),
                "`{}/{}` is not committed with the golden",
                golden.directory,
                file.path
            );
        }
    ...
    assert!(
        carried > 0,
        "no golden binds a `module:` implementation, so nothing in this corpus \
         type-checks an authored file against its generated contract"
    );
}
...
/// Every **generated** golden file carries the marker that says it is not
/// source, and every carried one does not (PRD §8, resolved q47).
///
/// Both halves, because the marker is what `build` reads to tell its own work
/// from an author's: a generated file that lost it is one a first build into
/// that directory would refuse to replace, and an authored file that gained it
/// is one a build would take for its own. The list a path is on decides which
/// question it is asked.
#[test]
fn every_golden_file_says_whose_it_is() {
    if regenerating() {
        return;
    }
    for golden in GOLDENS {
        ...
        let authored: BTreeSet<String> = emitted(golden)
            .carried()
            .iter()
        ...
        for path in files_under(&root) {
            ...
            if authored.contains(&path) {
                assert!(
                    ...
                    "`{}/{path}` is the author's and claims to be the compiler's",
                    golden.directory
                );
                continue;
            }
