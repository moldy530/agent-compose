...
import {
  flowPatchInputs,
  flowPatchNodeImplementOutput,
  flowPatchNodeReviewOutput,
} from "./schemas.ts";
...
/** `flow.patch` — the `input` root inside it (grammar 7.5). */
const flowPatchShape: runtime.Shape = {
  "properties": {
    "goal": "string"
  }
};
...
/**
 * `flow.patch` node `implement` — the `implement.output` root its guards read.
 */
const flowPatchNodeImplementShape: runtime.Shape = {
  "properties": {
    "summary": "string",
    "touched": {
      "items": "string"
...
/** `flow.patch` node `review` — the `review.output` root its guards read. */
const flowPatchNodeReviewShape: runtime.Shape = {
  "properties": {
    "feedback": "string",
    "verdict": "string"
  }
...
/**
 * `provider.openai` — an `openai` connection (grammar 12.1). Every value is read when a node calls it, never at import: a build carries no credential and `./index.ts` is where their presence is checked (PRD 5.9).
 */
const providerOpenai: runtime.ProviderBinding = {
  address: "provider.openai",
  kind: "openai",
  get apiKey(): string {
...
/** `model.reviewer` — `gpt-5-codex` on `provider.openai` (grammar 12.2). */
const modelReviewer: runtime.ModelBinding = {
  address: "model.reviewer",
  id: "gpt-5-codex",
...
const flowPatchNodeImplementCoder: runtime.HarnessBinding = {
  ...
  result: flowPatchNodeImplementOutput,
};

/**
 * `flow.patch` node `implement` — one `harness: cc` run on `model.implementer` (grammar 8.9).
 */
const flowPatchNodeImplement: runtime.NodeDescriptor = {
  flow: "flow.patch",
  node: "implement",
  ...
  shapes: { input: flowPatchShape, state: stateShape, output: flowPatchNodeImplementShape },
  input: (roots, view) => ({
...
const flowPatchNodeReviewCoder: runtime.HarnessBinding = {
  ...
  result: flowPatchNodeReviewOutput,
};

/**
 * `flow.patch` node `review` — one `harness: codex` run on `model.reviewer` (grammar 8.9).
 */
const flowPatchNodeReview: runtime.NodeDescriptor = {
  flow: "flow.patch",
  node: "review",
  ...
  shapes: { input: flowPatchShape, state: stateShape, output: flowPatchNodeReviewShape },
  input: (roots, view) => ({
    "goal": runtime.toJson(runtime.evaluate("input.goal", roots)),
  ...
  edges: [
    { to: "implement", when: "review.output.verdict == 'revise'", readsOutput: true, budget: { key: "flow.patch#2", max: 2 } },
    { to: END, otherwise: true },
  ],
};

/** `flow.patch` — its nodes, its `start` edges, and the compiled graph. */
function flowPatch() {
  return new StateGraph(State)
    .addNode("implement", (state: GraphState) => runtime.runNode(flowPatchNodeImplement, state), {
      ends: ["review"],
    })
    .addNode("review", (state: GraphState) => runtime.runNode(flowPatchNodeReview, state), {
      ends: ["implement", END],
    })
...
/**
 * `flow.patch`, compiled once. Building it at import is also what checks it: a state model LangGraph refuses, or an edge to a node that is not registered, fails here rather than at the first invocation.
 */
const flowPatchGraph = flowPatch();

/**
 * `flow.patch` as a module: what a `flow:` node instantiates and a `map` dispatches to (grammar 7.5, 8.5).
 */
const flowPatchBinding: runtime.SubflowBinding = {
  address: "flow.patch",
  outputs: ["summary"],
  recursionLimit: 31,
  stream: (initial, options) =>
    flowPatchGraph.stream(initial, {
      ...options,
      streamMode: "values",
      outputKeys: flowPatchGraph.outputChannels,
...
export const flows: Readonly<Record<string, CompiledFlow>> = {
  "flow.patch": {
    ...
    parse: (inputs: unknown) =>
      runtime.parseResult(flowPatchInputs, inputs, "the `inputs:` of `flow.patch`") as Record<string, unknown>,
    stream: (initial, options) =>
      flowPatchGraph.stream(initial, {
        ...options,
        streamMode: "values",
