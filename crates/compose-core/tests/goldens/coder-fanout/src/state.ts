...
export const channels = {
  /**
   * One entry per dispatched run, in source-item order.
   *
   * `reduce: append`: a write supplies **one element**, appended in canonical write order (grammar 7.6.4, Decision D58).
   * Starts at `[]`.
   */
  summaries: Annotation<z.infer<typeof stateSummaries>, runtime.Written<z.infer<typeof stateSummaries>[number]>>({
    reducer: (left, right) => runtime.appendReduce(left, right),
    default: () => [],
  }),
  /**
   * What one implementing run says it changed.
   *
  ...
  /**
   * The note the summarising run wrote in its own directory.
   *
   * Unreduced: single-writer, and a write supplies the whole value.
   * Starts at `""`.
   */
  survey: Annotation<z.infer<typeof stateSurvey>, runtime.Written<z.infer<typeof stateSurvey>>>({
    reducer: (left, right) => runtime.setReduce(left, right),
    default: () => "",
  }),
  /**
