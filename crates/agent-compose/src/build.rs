...
//!   header is not the compiler's to delete and stops the write instead — see
...
pub(crate) enum Refusal {
    ...
    Io(io::Error),
    /// `<out>/src/` holds files this compiler did not write, so the directory is
    /// not one it owns. The paths, sorted.
    NotOurs(Vec<String>),
}
...
///
/// So a file under `src/` that the emitter did not produce is removed only when
/// it carries [`GENERATED_MARKER`] — the header this compiler writes into every
/// file. One that does not stops the whole write with [`Refusal::NotOurs`], and
/// nothing has been deleted when it does, because the scan runs before any
/// removal. The removals that do happen come back in [`Written::removed`] and are
/// reported, so "the build also deleted three files" is something the run says
/// rather than something a later `git status` discovers.
