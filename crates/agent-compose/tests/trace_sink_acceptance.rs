...
//!
//! A **child execution** — what a detached `flow.*` dispatch starts (PRD
//! resolved q65) — ships through exactly the path every execution here does,
//! under its own id; its sink tests live with the rest of its contract in
//! `child_execution_acceptance.rs`, because detach is legal only under
//! `--target local` (Decision D59) and so needs a fixture of its own.
