...
fn build_writes_the_project_layout_where_out_points() {
    ...
    assert!(
        stderr(&output).contains("wrote 21 files"),
        "{}",
        stderr(&output)
    );
    assert_eq!(
...
"#,
    ...
    assert!(
        stub.contains("import type { toolSignInput, toolSignOutput } from \"../schemas.ts\";"),
        "the stub types itself against the tool's generated schemas: {stub}"
    );
    assert!(
    ...
    assert!(
        ...
        "the authored half is not part of what was emitted"
    );

    // 3. `validate` is clean now, and so is `--check`: the scaffold is not on
    //    the emitted file list, so nothing compares it.
    ...
    //    mentions it, and `--check` still passes.
    fs::write(
