...
- **Name tests and fixtures for what they verify, never for the milestone that introduced them** (`static_check_inventory.rs`, not `m0_inventory.rs`; `#[ignore = "pending: build command"]`, not `"M1: pending"`). Milestones are roadmap trivia; a filename should tell a reader what breaks when it fails without the PRD open beside it. PRD cross-references belong in test *contents*, where citing the requirement is the point.
